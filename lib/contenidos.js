...
export async function getEjercicios(categoria) {
  const url = `${STRAPI}/ejercicios/?categoria=${categoria}`
...
/**
* Obtiene el ejercicio con el slug recibido dentro de la categoria.
*/
export async function getEjercicio(categoria, slug) {
  const url = `${STRAPI}/ejercicios/${slug}?categoria=${categoria}`
...
/**
* Retorna los datos del curso.
*/
export async function getResumenCurso(slug) {
  const url = `${STRAPI}/masterclass/curso/${slug}`
...
export async function getResumenCategoria(categoria) {
  const url = `${STRAPI}/categorias/${categoria}/resumen`
  const resumen_res = await fetch(url)
  const resumen = await resumen_res.json()
...
export async function getCategorias() {
  ...
  const result = Promise.all(categorias.map(async (c) => {
    ...
    const categoria = {
      ...
      thumbnail: c.Thumbnail ? c.Thumbnail.url : "/img/ec_diferenciales.jpg",
      ejercicios: {
        q: resumen.q,
  ...
  const eachHijo = async (coll) => {
    ...
    for (let i = 0; i < coll.length; i++) {
      ...
      if (ruta.includes(categoria.Titulo_url)) {
        ...
        let base
        if (!paginas.length) {
          base =  `/${indice.Titulo_url}`
        } else {
          base = paginas[paginas.length - 1].url
        }
        const newUrl = `${base}/${categoria.Titulo_url}`
